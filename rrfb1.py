...
import csv
import time
cap = cv.VideoCapture("./SE_0800_1000.mp4")
if not cap.isOpened():
...
lightWork=False
start = time.time()
with open('cam2_1200-1500light.csv', mode='w') as light_file:
...
cap = cv.VideoCapture("cam2_1200-1500.mp4")
while True:
    ret, frame = cap.read()
    if not ret:
    ...
    with open('cam2_1200-1500light.csv', mode='a') as light_file:
        light_file = csv.writer(light_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
